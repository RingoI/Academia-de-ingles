...
import { Plus, Calendar, Users, GraduationCap, X, Layers, ChevronRight, Info, BookOpen, ArrowRight } from "lucide-react";
	...
	return (
		<div className="h-full w-full relative">
			...
			<header className="mb-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
				<div className="mb-1">
					<h1 className="font-semibold text-3xl text-slate-400">Administración de cursos</h1>
					<p className="text-slate-400">Configuración de oferta académica y cupos</p>
				</div>
						...
						>
							...
							<div className="relative z-10 flex-1">
								<div className="flex justify-between items-start mb-6">
									<h2 className="text-2xl font-bold text-white group-hover:text-blue-400 transition-colors pr-10">{curso.nombre}</h2>
								...
								<div className="space-y-4 mb-8">
									<div className="flex items-center gap-4 text-slate-400">
										...
										<span className="text-sm">
											Niveles: <b className="text-slate-200">{curso.niveles?.length ? curso.niveles.join(", ") : "N/A"}</b>
										</span>
									</div>
					...
					<div className="bg-slate-900 border border-slate-800 w-full max-w-xl rounded-[2.5rem] shadow-2xl p-10 animate-in fade-in zoom-in duration-300">
						...
						<form onSubmit={handleCrearCurso} className="space-y-6">
							...
							<div className="grid grid-cols-2 gap-6">
								...
								<div className="space-y-2">
									<label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Niveles</label>
									...
									<select
										...
										onChange={(e) =>
											setFormData({
												...formData,
												nivelesIds: Array.from(e.target.selectedOptions, (option) => Number(option.value)),
											})
										}
