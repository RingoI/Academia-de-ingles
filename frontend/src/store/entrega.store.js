...
export const entregaStore = create((set, get) => ({
	isUploading: false,
	entregas: [],
	historial: [],
	...
	subirEntregaTarea: async (tareaId, alumnoId, file, nombre) => {
		try {
			...
			const res = await axiosInstance.post(`/entregas/tarea/${tareaId}/alumno/${alumnoId}`, formData, {
				headers: {
	...
	subirEntregaExamen: async (examenId, alumnoId, file, nombre) => {
		try {
			...
			const res = await axiosInstance.post(`/entregas/examen/${examenId}/alumno/${alumnoId}`, formData, {
				headers: {
					"Content-Type": "multipart/form-data",
				},
			});
	...
	descargarEntrega: async (entregaId, nombreArchivo) => {
		try {
			const response = await axiosInstance.get(`/entregas/download/${entregaId}`, {
				responseType: "blob",
			});
	...
	entregasPorTarea: (tareaId) => get().entregas.filter((e) => e.tareaId === tareaId),
	...
	reentregar: async (entregaId, file, nombre) => {
		try {
			...
			const res = await axiosInstance.put(`/entregas/reentrega/${entregaId}`, formData, {
				headers: {
					"Content-Type": "multipart/form-data",
				},
			});
