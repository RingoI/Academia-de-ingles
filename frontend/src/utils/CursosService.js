...
export const CursosService = {
  ...
  asignarAlumno: async (cursoId, alumnoId) => {
    const response = await axiosInstance.post(
      `/cursos/${cursoId}/asignar-alumno/${alumnoId}`,
    );
    return response.data;
  },
  ...
  asignarDocente: async (cursoId, docenteId) => {
    const response = await axiosInstance.post(
      `/cursos/${cursoId}/asignar-docente/${docenteId}`,
    );
    return response.data;
  },
  ...
  desvincularAlumno: async (cursoId, alumnoId) => {
    const response = await axiosInstance.delete(
      `/cursos/${cursoId}/desvincular-alumno/${alumnoId}`,
    );
    return response.data;
  },
  desvincularDocente: async (cursoId, docenteId) => {
    const response = await axiosInstance.delete(
      `/cursos/${cursoId}/desvincular-docente/${docenteId}`,
    );
    return response.data;
  },
