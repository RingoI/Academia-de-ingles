...
import { Ban, CheckCircle2, SquarePen, WalletCards, XCircle } from "lucide-react";
...
function TablaAlumnos({ busqueda }) {
	...
	const cabecera = ["", "Nombre", "Email", "Dirección", "DNI", "Estado", "Acciones"];
	...
	return (
		<div className="overflow-x-auto">
			<div className={`${editarAlumno ? "fixed inset-0 ml-60" : "hidden"} z-10 flex items-center justify-center bg-black/50`}>
				<FormularioAlumnos
					key={datosAlumno.id}
			...
			<div className={`${abrirCuponera ? "fixed inset-0 ml-60" : "hidden"} z-10 flex items-center justify-center bg-black/50`}>
				<FormularioCuponera setAbrirCuponera={setAbrirCuponera} datosAlumno={datosAlumno} />
			</div>
					...
					{alumnosFiltrados.map((a, idx) => (
						<tr key={a.id}>
							...
							<td className="flex gap-2 text-slate-300">
								{a.estado ? (
									<span>
										<Ban className="size-5 cursor-pointer" onClick={() => modificarAlumno(a.id, { ...a, estado: false })} />
									</span>
								) : (
									<span>
										<CheckCircle2
											className="size-5 cursor-pointer hover:text-green-500"
											onClick={() => modificarAlumno(a.id, { ...a, estado: true })}
										/>
									</span>
								)}
								<span>
								...
								<span>
									<WalletCards
										className="size-5 cursor-pointer"
										onClick={() => {
											(setAbrirCuponera(!abrirCuponera), setDatosAlumno(a));
										}}
									/>
								</span>
							</td>
