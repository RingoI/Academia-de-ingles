...
function FormularioAlumnos({
  ...
  setAbrirFormularioAlumnos,
  values = {},
}) {
  const { agregarAlumno, actualizarAlumno } = usuarioStore();
  ...
  function handleSubmit(e) {
    ...
    if (!values.id) {
      ...
      actualizarAlumno(values.id, data); // Usamos 'data' procesado
    }
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400">
              {values.id
                ? "Edita los datos del estudiante seleccionado."
                : "Completa los datos para registrar al nuevo estudiante."}
            </p>
          </div>
        </div>
        <button
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 md:grid-cols-2 p-6 gap-6">
          ...
          <TextInput
            tag={"CONTRASEÑA"}
            placeholder={
              values.id ? "Dejar en blanco para no cambiar" : "••••••••"
            }
            type="password"
            onChange={(e) =>
              setFormularioAlumnos({
