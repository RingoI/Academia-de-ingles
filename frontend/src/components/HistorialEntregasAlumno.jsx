...
function HistorialEntregasAlumno({ alumnoId }) {
  ...
  const tareas = historial
    .filter((ent) => ent.tipo === "TAREA")
    .sort((a, b) => {
      const fecha = new Date(b.fechaEntrega) - new Date(a.fechaEntrega);
      if (fecha !== 0) return fecha;
      return b.id - a.id;
    });

  const examenes = historial
    .filter((ent) => ent.tipo === "EXAMEN")
    .sort((a, b) => {
      const fecha = new Date(b.fechaEntrega) - new Date(a.fechaEntrega);
      if (fecha !== 0) return fecha;
      return b.id - a.id;
    ...
    <div className="flex flex-col gap-8">
      ...
      {tareas.length > 0 && (
        <div className="flex flex-col gap-4">
          <h3 className="text-lg font-semibold text-cyan-400">📚 Tareas</h3>

          {tareas.map((ent) => {
            let estadoTexto = "Entregado";
            let estadoClase = "bg-blue-600/30 text-blue-300";
                  ...
                  <span className="text-xl font-bold text-green-400">
                    ...
                    {ent.nota !== null && ent.nota < 4 && (
                      <div className="flex flex-col items-end gap-2 mt-2">
                        ...
                        <button
                          onClick={() =>
                            handleReentrega(ent.id, archivos[ent.id])
                          }
      ...
      {examenes.length > 0 && (
        <div className="flex flex-col gap-4">
          <h3 className="text-lg font-semibold text-orange-400">📝 Exámenes</h3>
